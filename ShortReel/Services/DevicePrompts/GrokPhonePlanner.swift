...
enum GrokPhonePlanner {
    ...
    static func nextDecision(goal: String, frame: PhoneScreenFrame, history: [PhoneVisionStep]) async throws -> PhoneVisionDecision {
        ...
        let instructions = visualInstructions + """
            ...
            Additional named keyboard inputs: selectAll selects text in the focused field; addressBar focuses Safari's address bar with Command-L. These keys do not need a visual tap target. Home and search are also named system inputs: do not substitute an ungrounded tap on a Home indicator or icon. For app navigation, follow the shared Home → Spotlight → app-name → matching-result sequence, including when the app icon is visible. Use one input only, then inspect the next frame.
