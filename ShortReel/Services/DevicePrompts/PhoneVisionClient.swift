...
enum PhoneVisionClient {
    ...
    static let instructions = """
        ...
        press: one keyboard key: enter, escape, backspace, tab, search, selectAll, or addressBar. Search opens system phone Search with Command-Space. selectAll selects text in the focused field. addressBar sends Command-L to focus Safari's address field; use only when Safari is visibly open. For app launches, follow the Spotlight app-navigation sequence above. Home and Search are system inputs; they do not require a visible labeled button. Choose ONLY the next step and inspect again.
