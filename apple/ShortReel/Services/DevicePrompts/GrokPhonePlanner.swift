...
enum GrokPhonePlanner {
    ...
    static func nextDecision(goal: String, frame: PhoneScreenFrame, history: [PhoneVisionStep]) async throws -> PhoneVisionDecision {
        ...
        let instructions = visualInstructions + """
            ...
            Additional named keyboard inputs: selectAll selects text in the focused field; addressBar focuses Safari's address bar with Command-L. These keys do not need a visual tap target. Use AssistiveTouch to return Home: tap its floating button, inspect the menu, then tap Home. Never use the home action or a bottom-edge Home swipe. press/assistiveTouch opens the menu when its floating button cannot be located. For app navigation, prefer tapping the requested app’s visible Home/Dock icon at coordinates from the CURRENT screenshot. Use Spotlight only when the icon cannot be confidently located. Verify the focused search field before typing and the foreground app after tapping. Use one input only, then inspect the next frame.
    ...
    static func decodeResult(_ data: Data, goal: String, targets: [PhoneVisionClient.GroundingTarget]) throws -> PhoneVisionDecision {
        ...
        switch payload.kind {
        case .home:
            throw PhoneVisionError.invalidDecision("Return Home through AssistiveTouch: tap its floating button, inspect the menu, then tap Home.")
        case .tap:
