...
enum PhoneVisionClient {
    ...
    private static func decide(goal: String, context: ScreenContext, history: [PhoneVisionStep]) async throws -> PhoneVisionDecision {
        ...
        switch result.kind {
        case .home:
            throw PhoneVisionError.invalidDecision("Return Home through AssistiveTouch: open its menu, inspect it, then tap Home.")
        case .tap:
    ...
    static let instructions = """
        ...
        App navigation: first check whether the requested app is already running. If it is in the foreground, continue the user's task there. If the app switcher is showing and the app's card is visible, tap that card to bring it forward; do not go Home and do not relaunch. To launch, prefer tapping the requested app’s visible Home Screen or dock icon using a target from the CURRENT screenshot. If neither Home nor Spotlight is visible, use AssistiveTouch to return Home and inspect the next frame for the icon. Only when the icon cannot be confidently located on Home, choose swipe with direction down to open Spotlight or tap the visible Search control. This swipe starts below the status bar. Once Spotlight is visibly open with its search field focused, clear any previous query with press/selectAll, then typeText with only the requested app name. If the correct app name is already in the field, do not type it again. Wait for results if needed, then tap the matching visible app result using its unique OCR label; request input if the result is ambiguous or missing. Confirm the app is visibly open before continuing the rest of the goal, such as entering a website in Safari. Each of these is a separate turn with a fresh screenshot; never assume Home, Spotlight, typing, or launch succeeded. Follow an explicitly requested icon tap or exact-coordinate input as given.
        Available kinds:
        home: unused. Return Home through AssistiveTouch’s visible controls instead. Going Home leaves the app running; it does not force-quit the app or close its tabs.
        ...
        typeText: type at most 100 characters into the visibly focused input. Use text supplied by the user, an app name needed for the goal, or text the user explicitly asked you to compose. Never invent credentials or compose unsolicited messages. If no field is focused, first tap the field and observe the next screenshot.
        press: one keyboard key: enter, escape, backspace, tab, search, selectAll, addressBar, or assistiveTouch. Search opens system phone Search with Command-Space. selectAll selects text in the focused field. addressBar sends Command-L to focus Safari's address field; use only when Safari is visibly open. For app launches, prefer the visible app icon and use Spotlight as the fallback described above. AssistiveTouch and Search are system inputs; they do not require a visible labeled button. Choose ONLY the next step and inspect again.
