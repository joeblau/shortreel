...
final class BluetoothHIDHost: DeviceHost {
    ...
    func swipe(from start: NormalizedPoint, to end: NormalizedPoint, on device: DeviceDescriptor) async throws {
        try await drag(from: start, to: end, duration: 0.3, pressDuration: 0, holdDuration: 0, on: device)
    }

    func doubleTap(_ point: NormalizedPoint, on device: DeviceDescriptor) async throws {
        try await tap(point, on: device)
    ...
    func longPress(_ point: NormalizedPoint, seconds: Double, on device: DeviceDescriptor) async throws {
        try ensureConnected(device)
        try Task.checkCancellation()
        guard seconds.isFinite, (0.2...3).contains(seconds),
              [point.x, point.y].allSatisfy({ $0.isFinite && (0...1).contains($0) }) else {
            throw DeviceHostError.unsupportedInput("Invalid long-press coordinates or duration.")
        }
        defer { try? send(mouseReport(buttons: 0, point: point), to: device) }
        try send(mouseReport(buttons: 0, point: point), to: device)
    ...
    func drag(from start: NormalizedPoint, to end: NormalizedPoint, duration: Double,
        ...
        try ensureConnected(device)
        try Task.checkCancellation()
        guard [start.x, start.y, end.x, end.y].allSatisfy({ $0.isFinite && (0...1).contains($0) }),
              start != end, duration.isFinite, (0.1...3).contains(duration),
              pressDuration.isFinite, (0...3).contains(pressDuration),
              holdDuration.isFinite, (0...3).contains(holdDuration) else {
            throw DeviceHostError.unsupportedInput("Invalid drag coordinates or timing.")
        }
        try await HIDPointerDrag.run(from: start, to: end, duration: duration,
                                     pressDuration: pressDuration, holdDuration: holdDuration) { down, point in
            try self.send(self.mouseReport(buttons: down ? 1 : 0, point: point), to: device)
        }
    }
    ...
    func pressKey(_ key: DeviceKeyboardKey, on device: DeviceDescriptor) async throws {
        ...
        case .shiftTab: (code, modifiers) = (0x2B, 0x02)
        case .space: (code, modifiers) = (0x2C, 0)
        case .deleteForward: (code, modifiers) = (0x4C, 0)
        case .arrowRight: (code, modifiers) = (0x4F, 0)
    ...
    private func goHome(on device: DeviceDescriptor) async throws {
        ...
        for step in 1...steps {
            ...
            let eased = t * t * (3 - 2 * t)
            let point = NormalizedPoint(x: start.x, y: start.y + (end.y - start.y) * eased)
            try send(mouseReport(buttons: 1, point: point), to: device)
            lastPoint = point
        }
    ...
    func openAppSwitcher(on device: DeviceDescriptor) async throws {
        try ensureConnected(device)
        try Task.checkCancellation()
        let start = NormalizedPoint(x: 0.5, y: 1.0)
        let end = NormalizedPoint(x: 0.5, y: 0.55)
        defer { try? send(mouseReport(buttons: 0, point: end), to: device) }
        ...
        for step in 1...steps {
            ...
            let t = Double(step) / Double(steps)
            let eased = t * t * (3 - 2 * t)
            try send(mouseReport(buttons: 1, point: .init(x: start.x, y: start.y + (end.y - start.y) * eased)), to: device)
        }
