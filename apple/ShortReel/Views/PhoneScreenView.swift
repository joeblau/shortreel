        ...
        InspectorSection("Phone Screen") {
            if capture.latestFrame != nil {
                HStack(spacing: 8) {
                    ...
                    Text(deviceManager.visionProvider == .onDevice
                        ? "Live screen · processed on this Mac" : "Live screen")
                        .font(.callout)
            ...
            if !capture.isRunning {
                if capture.sources.isEmpty {
                    ...
                    Picker("Screen", selection: $selectedSourceID) {
                    ...
                    .labelsHidden()
                    .accessibilityLabel("Screen for \(device.name)")
                }
            }
