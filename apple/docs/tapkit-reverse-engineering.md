...
The Device inspector's Planner setting selects **UI-TARS** or **On this Mac** for
screen-based requests. The selection is saved on this Mac and defaults to the
UI-TARS planner. Changing it stops active requests; the picker is disabled while
...
- **UI-TARS** uses the built-in local model or the model service configured in
  `~/.ui-tars-cli.json`. Each request uses only the selected planner.
