...
`DeviceManager` first identifies the trusted USB phone by its Bluetooth address. Automatic association prefers a unique physical UDID match when one is exposed. For privacy UUIDs, it requires exactly one USB phone and one genuine iOS screen source with matching names. Names alone or source ordering cannot select a screen among multiple phones; an explicit screen-picker selection supplies that association.

The capture engine keeps AVFoundation objects on one serial queue. It converts sample presentation timestamps through the capture session's synchronization clock to distinguish newly captured frames from late delivery. It encodes at most three JPEG frames per second, bounded to a 1280-pixel long edge. Published dimensions come from the encoded image.
